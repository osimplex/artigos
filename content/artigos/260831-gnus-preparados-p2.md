...
Com outra outra linguagem: _o direito é uma função do vínculo_, ou uma estrutura abstrata de articulação entre indivíduos.
...
A questão do direito autoral tem dimensões. A humanidade compõe música, pinta, conta histórias desde que provavelmente o período das cavernas, e embalados pelas diversas formas de expressão nos embalamos enquanto sociedade. E nessa história houveram miríades de miríades de criações cujos autores sequer sabemos quem são, seja porque sequer chegou um nome, seja por causa do que chamamos pseudoepígrafe, uma falsa atribuição de autoria. Mas recebemos e transmitimos, fenômeno que chamamos _tradição_, para que continuemos enquanto sociedade.
...
A história continuou e ganhou outros capítulos. A noção do _copyright_ incluiu a música quando, após o surgimento da possibilidade de gravar e reproduzir sons, veio a indústria fonográfica. Hoje o escopo do _copyright_ alcança diversas formas de expressão humana, e isso inclui o _software_.
 ...
 * ND --- É vedada a produção de obras derivas e adaptações;
...
Por outro lado, alguns polemistas, fomentando tensão, opõe licenças com _copyleft_ às "permissivas" preterindo as primeiras. Entre os recursos retóricos que surgem está o de apresentar as licenças com _copyleft_ como se fossem "menos livres", "restritivas", "tirassem" alguma liberdade.
...
Mas saindo da retórica dos detratores, há uma motivação material. Eles não aderem a um licenciamento de _software_ livre, ou de código aberto, que requer a manutenção de intenção do autor original nas obras derivadas, a intenção de que haja liberdade de _software_ para todos os que tiverem contato com a obra (pensamento ligado ao _software_ livre), ou a preservação dos condições do modo de produção do _software_ (pensamento ligado ao código aberto).
...
Com os LLMs ficou mais barato emular esse processo quando o objetivo é violar os direitos de quem, diferente da _Apple_, não conta com um exército particular de advogados e sabe-se lá o que mais na folha de pagamento. Pode-se requisitar uma implementação parelha com instruções específicas para gerar diferenças, e, dado que o direito autoral é sobre a expressão, calculando uma sequência de palavras com diferença suficientemente que faça a mesma coisa, é possível passar a obra derivada por original. Fazer isso com seres humanos seria bastante trabalhoso, mas com uma super-calculadora à disposição a situação muda.
...
Essa questão de liberdade e licenciamento pode não ser tão valorizada por aí, mas se não fosse pelo que foi exposto neste artigo não haveria este série. Seja o leitor da linha que for, que valoriza antes o modo de produção de _software_ ou a liberdade na computação, isso foi viabilizado pelo _hack_ legal que nos legou uma fabulosa subversão do direito autoral.
...
Nos próximos artigos prosseguirmos, no gozo da plenitude das liberdades de _software_, o caminho para nosso domínio intelectual do sistema operacional.
